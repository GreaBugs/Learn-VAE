...
dataset = load_dataset("Datasets", split="train", data_dir="")  # 也可以这样加载本地文件夹的图片数据集
...
for epoch in range(num_epochs):
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        data = batch["images"].to(device)  # [bs, c, h, w] 的原始图像张量 (bs, 3, 512, 512)

        recon_batch, _, mu, logvar = vae(data)  # 传递给VAE模型，获取重构图像、均值和对数方差  (8, 3, 512, 512)
