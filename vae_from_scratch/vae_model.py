...
class VAE(nn.Module):
    ...
    def encode(self, input):
        result = self.encoder(input)  # (8, 256, 64, 64)
        mu = self.fc_mu(result)  # (8, 4, 64, 64)
        log_var = self.fc_var(result)  # (8, 4, 64, 64)
        return mu, log_var

    def decode(self, z):
        result = self.decoder_input(z)  # (8, 256, 64, 64)
        result = self.decoder(result)  # (8, 3, 512, 512)
        # result = self.sigmoid(result)  # 如果原始图像被归一化为[0, 1]，则使用sigmoid
        result = self.tanh(result)  # 如果原始图像被归一化为[-1, 1]，则使用tanh    # (8, 3, 512, 512)
    ...
    def forward(self, input):
        ...
        """
        mu, log_var = self.encode(input)  # (8, 3, 512, 512) -> (8, 4, 64, 64), (8, 4, 64, 64)
        z = self.reparameterize(mu, log_var)  # 潜在空间的向量表达 Latent Vector z  (8, 4, 64, 64)
